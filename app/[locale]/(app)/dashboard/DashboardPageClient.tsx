...
export function DashboardPageClient({isDemoPatient}: DashboardPageClientProps) {
  ...
  const [isLoading, setIsLoading] = useState(!isDemoPatient);
  ...
  return (
    <div className="mx-auto flex w-full max-w-4xl flex-col gap-8">
      ...
      {!isLoading && unreadCount > 0 && (
        <div role="status" className="flex items-center justify-between rounded-md border border-border bg-card p-4">
          <p className="text-sm">{t("unreadMessages", {count: unreadCount})}</p>
          <Button type="button" className="h-8 px-3 text-xs" onClick={() => router.push(`/${locale}/messages`)}>
